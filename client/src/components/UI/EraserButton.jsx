...
const EraserButton = ({width, height, setOpen, open, parentRef, figures}) => {
    const dispatch = useDispatch();
    const currentTool = useSelector(state => state.tool.toolsSetting.currentTool)
    const lineWidth = useSelector(state => state.tool.toolsSetting['eraser'].width)
    ...
    return (
        <div ref={childRef}>
        <button className='toolbar__btn' onClick={() => {dispatch(setCurrentTool('eraser')); setOpen(!open); figures(false)}}>
        ...
        <div className='toolbar__brush__setting'>
            <label htmlFor="brush-range">eraser width: </label>
                <input 
                type="range" 
                min={1} 
                max={50} 
